...
public class BilleteraTest {
    ...
    public void testInversionYSaldoDisponible() {
        String cvuAlice = billetera.crearCuentaPremium("11111111", "alice.p", 2000000); // crea cuenta premium con 2M de deposito inicial. Usuario 11111111

        billetera.realizarInversionRentaFija("11111111", cvuAlice, 500000, 30); // Realiza inversion de renta fija asociada a la cuenta 'alice.p' para el usuario 11111111
        double invertidoMasDisponible = billetera.obtenerSaldoDisponible(cvuAlice) // Suma el saldo de la cuenta alice.ph con todo lo que invirtió el usuario 11111111
                + billetera.obtenerTotalInvertido("11111111");

        assertEquals(500000.0, billetera.obtenerTotalInvertido("11111111"), 0.01); 		// La inversion previa de 500000 del usuario 11111111 debio ser registrada. El total tiene que ser 500000 
        assertEquals(1500000.0, billetera.obtenerSaldoDisponible(cvuAlice), 0.01); 		// eran 2M, se invirtieron 500k, quedan 1.5M
        assertEquals(2000000.0, invertidoMasDisponible, 0.01);							// lo que queda en al cuenta alice.p (1.5M) + lo que invirtió 11111111 (500k) tiene que dar 2M.

        billetera.realizarInversionDivisa("11111111", cvuAlice, 100000, 60, "USD", 0.05); // ahora el mismo usuario vuelve a invertir 100k en una inversion vinculada a divisa.
        assertEquals(600000.0, billetera.obtenerTotalInvertido("11111111"), 0.01);			// entonces invirtió 500K antes y 100K ahora, el total es 600k.
    }
